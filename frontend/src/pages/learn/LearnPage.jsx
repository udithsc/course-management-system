...
import {
  ...
  ListItemIcon, Divider, Button, LinearProgress, Chip,
...
import { loadMyCourses, markVideoWatched, selectMyCourses, selectCourseProgress } from '../../store/subscriptions';
...
export default function LearnPage() {
  ...
  const curriculum = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
        <Typography variant="subtitle2" fontWeight={800} noWrap>{course?.name ?? 'Course Curriculum'}</Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.75 }}>
          <LinearProgress variant="determinate" value={progress} sx={{ flex: 1, height: 5, borderRadius: 3,
            bgcolor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.06)',
            '& .MuiLinearProgress-bar': { bgcolor: '#10B981', borderRadius: 3 } }} />
          <Typography variant="caption" fontWeight={800} color="success.main">{progress}%</Typography>
        </Box>
      </Box>
              ...
              <React.Fragment key={lesson.id}>
                <ListItemButton
                  ...
                  sx={{
                    ...
                    '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(15,23,42,0.03)' },
                  }}
                >
                  ...
                  <ListItemText
                    primary={
                      <Typography variant="body2" fontWeight={active ? 700 : 500}
                        color={active ? 'primary.main' : 'text.primary'} noWrap>
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
      {/* ── Desktop Sidebar ──────────────────────────────────────────────── */}
      {!isMobile && (
        <Box sx={{ width: SIDEBAR_W, flexShrink: 0, borderRight: '1px solid', borderColor: 'divider', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          {curriculum}
        </Box>
      )}
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Video player */}
        <Box sx={{ flex: 1, bgcolor: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          {activeLesson?.url
            ? (
              <video
                ref={videoRef}
                ...
                style={{ width: '100%', height: '100%', maxHeight: '70vh', objectFit: 'contain', outline: 'none' }}
              >
        ...
        {activeLesson?.description && (
          <Box sx={{ px: 3, py: 2.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', maxHeight: 140, overflowY: 'auto', flexShrink: 0 }}>
            <Typography variant="subtitle2" fontWeight={800} gutterBottom>About this lesson</Typography>
