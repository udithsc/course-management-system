...
import {
  ...
  Chip, Skeleton, useTheme, Button, Divider,
} from '@mui/material';
...
import { selectMyCourses, selectIsEnrolled, selectCourseProgress, loadMyCourses } from '../../store/subscriptions';
...
export default function ExplorePage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      >
        {/* decorative circles */}
        {[{ top: -60, right: -60, size: 240 }, { bottom: -80, left: -80, size: 300 }].map((b, i) => (
          <Box key={i} sx={{ position: 'absolute', top: b.top, right: b.right, bottom: b.bottom, left: b.left,
            width: b.size, height: b.size, borderRadius: '50%', bgcolor: 'rgba(255,255,255,0.05)' }} />
        ))}
      ...
      <Box sx={{ px: { xs: 2, md: 4 }, py: 4, maxWidth: 1400, mx: 'auto' }}>
        ...
        {myCourses.length > 0 && (
          <Box sx={{ mb: 5 }}>
            <Typography variant="h6" fontWeight={800} sx={{ mb: 0.5 }}>📚 My Learning</Typography>
        ...
        <Grid container spacing={2.5}>
          {loading
            ? Array.from({ length: 8 }).map((_, i) => (
                <Grid item key={i} xs={12} sm={6} md={4} lg={3}><CardSkeleton /></Grid>
              ))
            : filtered.length === 0
              ? (
                <Grid item xs={12}>
                  <Box sx={{ textAlign: 'center', py: 8 }}>
                    ...
                    <Button onClick={() => { setSearch(''); setActive('all'); }} sx={{ mt: 2 }}>
                      Clear filters
                    </Button>
                  </Box>
                </Grid>
