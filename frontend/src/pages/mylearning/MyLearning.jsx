...
import {
  Box, Grid, Typography, LinearProgress, Button,
...
import {
  loadMyCourses, selectMyCourses, selectSubsLoading, selectCourseProgress,
  ...
  return (
    <MotionBox
      ...
      sx={{
        ...
        '&:hover': { transform: 'translateY(-3px)', boxShadow: `0 12px 32px rgba(0,0,0,0.1), 0 0 0 1px ${accent}33` },
      }}
    >
      {/* Thumbnail */}
      <Box sx={{ position: 'relative', height: 140, bgcolor: `${accent}15` }}>
        ...
        {progress === 100 && (
          <Box sx={{ position: 'absolute', inset: 0, bgcolor: 'rgba(16,185,129,0.85)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <EmojiEventsIcon sx={{ fontSize: 36, color: 'white', mb: 0.5 }} />
            <Typography variant="caption" sx={{ color: 'white', fontWeight: 800 }}>Completed!</Typography>
          </Box>
        )}
        {course.category?.name && (
          <Chip label={course.category.name} size="small"
            sx={{ position: 'absolute', top: 8, left: 8, bgcolor: accent, color: 'white',
              fontWeight: 700, borderRadius: '6px', fontSize: '0.65rem', height: 20 }} />
        )}
      </Box>

      {/* Body */}
      <Box sx={{ p: 2 }}>
        <Typography variant="body2" fontWeight={700} sx={{ mb: 0.75, display: '-webkit-box',
        ...
        {course.author && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1.5 }}>
            <Avatar sx={{ width: 18, height: 18, bgcolor: `${accent}22`, color: accent, fontSize: '0.6rem', fontWeight: 800 }}>
              {course.author.name?.charAt(0)}
            </Avatar>
            <Typography variant="caption" color="text.secondary" noWrap>{course.author.name}</Typography>
          </Box>
        )}
        ...
        <Box sx={{ mb: 1.5 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
            ...
            <Typography variant="caption" fontWeight={800} sx={{ color: accent }}>{progress}%</Typography>
          </Box>
          <LinearProgress variant="determinate" value={progress}
            sx={{ height: 6, borderRadius: 3, bgcolor: `${accent}18`,
        ...
        <Button
          ...
          size="small"
          sx={{
            borderRadius: '8px', fontWeight: 800, py: 0.9,
            ...(progress < 100 && {
        ...
        >
          {progress === 0 ? 'Start Learning' : progress === 100 ? 'Review Course' : `Continue · ${remaining} left`}
        </Button>
      </Box>
    </MotionBox>
  );
...
export default function MyLearning() {
  ...
  const completed  = myCourses.filter((c) => c.watchedVideoId?.length === c.lessons?.length && c.lessons?.length > 0);
  const inProgress = myCourses.filter((c) => (c.watchedVideoId?.length ?? 0) > 0 && c.watchedVideoId?.length < c.lessons?.length);
  ...
  return (
    <Box>
      {/* Header */}
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight={900} gutterBottom>My Learning</Typography>
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          {[
            ...
            <Grid item key={s.label} xs={4}>
              <Box sx={{ p: 2.5, borderRadius: '10px', border: '1px solid', borderColor: 'divider',
      ...
      {!loading && myCourses.length === 0 && (
        <Box sx={{ textAlign: 'center', py: 10 }}>
          ...
          <Typography variant="h5" fontWeight={800} gutterBottom>No courses yet</Typography>
          ...
          <Button variant="contained" size="large" onClick={() => navigate('/explore')}
            sx={{ borderRadius: '10px', fontWeight: 800, background: 'linear-gradient(135deg, #6366F1, #4F46E5)' }}>
            Explore Courses
          </Button>
        </Box>
      )}
      ...
      {!loading && inProgress.length > 0 && (
        <Box sx={{ mb: 4 }}>
          <Typography variant="h6" fontWeight={800} sx={{ mb: 2 }}>⏱ In Progress</Typography>
          <Grid container spacing={2.5}>
            {inProgress.map((c, i) => (
              <Grid item key={c.id} xs={12} sm={6} md={4}><ProgressCard course={c} index={i} /></Grid>
            ))}
          </Grid>
        </Box>
      )}
      ...
      {!loading && notStarted.length > 0 && (
        <Box sx={{ mb: 4 }}>
          <Typography variant="h6" fontWeight={800} sx={{ mb: 2 }}>🚀 Not Started</Typography>
          <Grid container spacing={2.5}>
            {notStarted.map((c, i) => (
              <Grid item key={c.id} xs={12} sm={6} md={4}><ProgressCard course={c} index={i} /></Grid>
            ))}
      ...
      {!loading && completed.length > 0 && (
        <Box>
          <Typography variant="h6" fontWeight={800} sx={{ mb: 2 }}>🏆 Completed</Typography>
          <Grid container spacing={2.5}>
            {completed.map((c, i) => (
              <Grid item key={c.id} xs={12} sm={6} md={4}><ProgressCard course={c} index={i} /></Grid>
            ))}
