  ...
  return (
    <Box sx={{ mt: 1 }}>
      {configData.ROUTES.map((route) => {
        const isParentSelected = route.subMenu.length === 0 
        ...
        <Box key={route.path}>
          ...
          >
            <ListItemIcon sx={{ minWidth: 40 }}>{GenerateIcon(route.icon, isParentSelected)}</ListItemIcon>
            <ListItemText 
              primary={route.title} 
          ...
          {route.subMenu.length > 0 && (
            <Collapse in={nestedMenuOpen} timeout="auto" unmountOnExit>
              <List component="div" disablePadding sx={{ position: 'relative', '&::before': {
                content: '""',
                position: 'absolute',
                left: '28px',
                top: 0,
                  ...
                  >
                    ...
                    <ListItemText 
                      ...
                      primaryTypographyProps={{ 
                        ...
                        color: selectedRoute === subRoute.path ? 'primary.main' : 'text.secondary'
                      }} 
                    />
                  </MainListItem>
                ))}
